...
@test "We should be able to create a cloud provider" {
  ...
  [[ "$status" -eq 0 ]]
  runit "kubectl -n ${NAMESPACE} get provider default"
  [[ "$status" -eq 0 ]]
}

@test "We should have a healthy cloud provider" {
  runit "kubectl -n ${NAMESPACE} get provider default -o json" "jq -r '.status.conditions[0].name' | grep -q 'Provider Ready'"
  [[ "$status" -eq 0 ]]
  runit "kubectl -n ${NAMESPACE} get provider default -o json" "jq -r '.status.conditions[0].status' | grep -q True"
  [[ "$status" -eq 0 ]]
}
