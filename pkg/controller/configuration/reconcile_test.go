...
var _ = Describe("Configuration Controller", func() {
	...
	verifyPolicyArguments := []string{
		...
		"--command=/usr/local/bin/checkov --config /run/checkov/checkov.yaml --framework terraform_plan -f /run/plan.json --soft-fail -o json -o cli --output-file-path /run --repo-root-for-plan-enrichment . >/dev/null",
...
`
	...
	When("checkov with an external source", func() {
		...
		It("should have updated the command line for checkov scan", func() {
			...
			expected := []string{
				"--comment=Evaluating Against Security Policy",
				"--command=/usr/local/bin/checkov --config /run/checkov/config.yaml --framework terraform_plan -f /run/plan.json --soft-fail -o json -o cli --output-file-path /run --repo-root-for-plan-enrichment . >/dev/null",
				"--command=/bin/cat /run/results_cli.txt",
				"--namespace=$(KUBE_NAMESPACE)",
				"--upload=$(POLICY_REPORT_NAME)=/run/results_json.json",
				"--is-failure=/run/steps/terraform.failed",
