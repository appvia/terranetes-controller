...
func main() {
	...
	flags.BoolVar(&config.EnableWebhookRegistration, "enable-webhook-registration", true, "Indicates we should register the webhooks")
	flags.BoolVar(&config.DisableWebhooks, "disable-webhooks", false, "Disables the webhook and mutation handlers")
	...
	flags.StringVar(&config.TerraformImage, "terraform-image", "hashicorp/terraform", "The namespace which the jobs should run in")
	flags.StringVar(&config.TerraformVersion, "terraform-version", "1.1.7", "The tag of the terraform image to use")
